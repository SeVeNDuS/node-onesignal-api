...
Notifications.prototype.track = function (notification_id, app_id, opened, callback) {
    ...
    this.request('PUT', ['notifications', notification_id], opened, {
        app_id: app_id, // string required
...
Notifications.prototype.create = function (restApiKey, params, callback) {
    ...
    var contentsRequired = (!params.content_available && !params.template_id);
    if (contentsRequired && !params.contents) {
        return callback(new Error('OneSignal Error (contents parameter required)'));
    }
    if (!contentsRequired && !params.content_available && !params.template_id) {
        return callback(new Error('OneSignal Error (content_available or template_id are required)'));
    }
